...
import { copy } from '../../utils/react/clipboard'
import { convertDenomToReadable } from '../../utils/blockchain/convertDenomToReadable'
import { getShortAddress } from '../../utils/blockchain/getShortAddress'
import { truncateLongString } from '../../utils/js/truncateLongText'
import { useWallet } from '../../utils/blockchain/wallet'
                ...
                {balances?.map((val) => (
                  <span key={`balance-${val.denom}`}>
                    ...
                    {tokensList.find((t) => t.denom === val.denom)?.displayName
                      ? tokensList.find((t) => t.denom === val.denom)?.displayName
                      : truncateLongString(val.denom ? val.denom : '', 28)}
                  </span>
                ))}
              </div>
