...
export default function Raffle() {
  ...
  const [
    ...
    // t('raffle:steps', { returnObjects: true })
    ["Select NFTs", "Raffle Details", "Confirm Raffle"];
  ...
  const getStepSchema = (currentStep: 1 | 2) => {
    const formSchemas = {
      [CREATE_RAFFLE_LISTING_FORM_STEPS.SELECT_NFTS]:
        RafflesSelectNFTStepSchema,
      [CREATE_RAFFLE_LISTING_FORM_STEPS.RAFFLE_DETAILS]:
        RaffleDetailsStepSchema,
    };
  ...
  const onSubmit: SubmitHandler<RaffleFormStepsProps> = async ({
    ...
    const data: {
      ...
      transactionAction: createRaffleListing(selectedNFTs, ticketPrice, {
        rafflePreview: selectedNFTs.findIndex(
          (nft) =>
            `${coverNFT.collectionAddr}_${coverNFT.tokenId}` ===
            `${nft.collectionAddr}_${nft.tokenId}`
        ),
    ...
    if (data) {
      const { raffleId, explorerUrl } = data;
  ...
  return (
    <Page
      title="New Raffle" // {t('common:title')}
    >
      <LayoutContainer>
        <TradeBackgroundLogoContainer>
          <TradeBackgroundLogo />
